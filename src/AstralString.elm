module AstralString exposing (filter, fromList, length, map, oneChar, reverse, toList)
...
toList =
    Regex.find oneChar >> List.map .match
...
length : String -> Int
length =
    Regex.find oneChar >> List.length
